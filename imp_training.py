# train_picolt_vit_full.py (updated to use Improved PICoLT + Perceptual Loss)
import json
import os
import h5py
import numpy as np
...
# === CHANGED: import from improved checker ===
from imp_picolt_checker import build_improved_picolt, forward_lens, PerceptualLoss
...
with open(SPLITS_JSON, "r") as jf:
    splits_map = json.load(jf)   # expects keys 'train','val','test'
...
# Build model + losses
# -------------------------
# === CHANGED: use improved model ===
model = build_improved_picolt(image_size=128, param_dim=len(param_cols))
opt = optimizers.Adam(1e-4)

mse = keras.losses.MeanSquaredError()
mae = keras.losses.MeanAbsoluteError()
perceptual_loss = PerceptualLoss()   # === NEW ===
...
def train_step(lensed, source, params,
    ...
    with tf.GradientTape() as tape:
        ...
        # === CHANGED: source loss includes perceptual ===
        loss_src_l1 = mae(source, source_pred)
        loss_src_ssim = ssim_loss(source, source_pred)
        loss_src_perc = perceptual_loss(source, source_pred)
        loss_src = 0.4*loss_src_l1 + 0.3*loss_src_ssim + 0.3*loss_src_perc
